...
import { AutoResizeTextarea } from '@/components/employee/WorkLogEntryTable';
...
// Manager may only ever log against the Service PO container itself here (a future capability
...
// list in ManagerFillWorkLog.jsx; the parent mounts this with `key={employee.id}` so switching
// Employees always starts from a fresh instance instead of carrying over stale entries/errors.
// Distinct from Timesheet Approval (ManagerTimesheetApproval.jsx), which only approves/rejects
...
// shared filter — a Manager backfilling one Employee's earlier month shouldn't have to change
...
// rather than assumed.
const ManagerFillWorkLogDrawer = ({ employee, monthYear: initialMonthYear, open, onOpenChange }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // instead of letting the Manager retry the exact same now-guaranteed-to-fail submission.
                      ...
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[7rem_1fr]">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <AutoResizeTextarea
                            ...
                            onChange={(e) => handleEntryChange(entry.service_po_id, 'description', e.target.value)}
...
export default ManagerFillWorkLogDrawer;
