              ...
              <table className="min-w-max w-full border-collapse text-sm">
                ...
                <thead className="sticky top-0 z-20">
                  ...
                  <tr className="border-b bg-muted/40">
                    <th className={th('sticky left-0 z-30 bg-muted w-[40px] text-center')}>#</th>
                    <th className={th('sticky left-[40px] z-30 bg-muted min-w-[180px]')}>Name</th>
                    <th className={th('min-w-[140px]')}>Designation</th>
                    <th className={th('w-[76px] text-right')}>Total Exp</th>
                    <th className={th('w-[76px] text-right')}>Co. Exp</th>
                    <th className={th('w-[90px] text-right')}>Cap. (hrs)</th>
                    <th className={th('w-[90px] text-right')}>Bill. Cap.</th>
                    <th className={th('min-w-[150px]')}>Client(s)</th>
                    {NON_BILLABLE_COLS.map((c) => (
                      <th key={c.key} className={th('w-[120px] text-right bg-orange-500/[0.04]')}>{c.label}</th>
                    ))}
                    {OTHER_COLS.map((c) => (
                      <th key={c.key} className={th('w-[76px] text-right')}>{c.label}</th>
                    ))}
                    <th className={th('w-[110px] text-right bg-blue-500/[0.04] font-bold')}>Billable (hrs)</th>
                    <th className={th('w-[110px] text-right bg-blue-500/[0.04] font-bold')}>Non-Bill. (hrs)</th>
                    <th className={th('w-[160px] text-right bg-blue-500/[0.04] font-bold border-r-0')}>Total Utilization (excl. Leaves)</th>
                  </tr>
                </thead>
                ...
                <tbody className="divide-y divide-border">
                  {records.map((row, i) => (
                    <tr key={row.employee_id ?? i} className="group hover:bg-muted/30 transition-colors">
                      {/* # */}
                      <td className={td('sticky left-0 z-10 bg-background group-hover:bg-muted/30 w-[40px] text-center text-muted-foreground transition-colors')}>
                        {((meta.page ?? page) - 1) * (meta.limit ?? 10) + i + 1}
                      </td>
                      {/* Name */}
                      <td className={td('sticky left-[40px] z-10 bg-background group-hover:bg-muted/30 transition-colors')}>
                      ...
                      <td className={td('max-w-[170px] truncate')} title={row.clients}>{row.clients || <span className="text-muted-foreground">—</span>}</td>
                      {/* Non-Billable */}
                      {NON_BILLABLE_COLS.map((c) => (
